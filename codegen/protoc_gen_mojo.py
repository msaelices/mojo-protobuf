...
def _enum_constants(fd, package):
    """Emit each enum's named values as `comptime <Enum>_<VALUE> = Int32(n)`.

    Covers top-level and nested enums; the enum name is flattened the same way
    as nested messages (`Outer_Inner`).
    """
    ...
    def emit(enums, scope):
        for e in enums:
            ...
            for v in e.value:
                lines.append(f"comptime {flat}_{v.name} = Int32({v.number})")
