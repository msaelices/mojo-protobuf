...
`Int`, `Int32`, `Int64`, `UInt32`, `UInt64`, `Bool`, `String`, `Float32`, and
`Float64` (the machine-width `Int` maps to an `int64` varint). Any other type is
a compile error (unless you override the methods). `decode` default-constructs the message, so
